...
colnames(otu_table_controls) <- gsub("DIVINTERNAL.", "", colnames(otu_table_controls))

# Calculate the relative abundance (not rarefied)
#otu_table_controls2 <- otu_table_controls %>%
#  column_to_rownames("taxon_column")
...
#filtering negaives exxtras
#otu_table_controls_top20 <- otu_table_controls_top20 %>% select(contains("EXT"))
#otu_table_controls_top20 <- otu_table_controls_top20 %>% select(contains("NEG"))
...
otu_table_controls_top20$Control_Type <- ifelse(grepl("BS.", rownames(otu_table_controls_top20)), "Boostershot Positive",
                                                ifelse(grepl("BS.NEG", rownames(otu_table_controls_top20)), "Boostershot Negative",
...
otu_table_controls_top20$Control_Type <- factor(otu_table_controls_top20$Control_Type,
                                                levels = c("BoosterShot Positive", "BoosterShot Negative",
...
# REMOVE NEGATIVES
#otu_table_controls_top20 <- select(otu_table_controls_top20, -contains("NEG"))
...
#otu_table_controls_top20_melt <- otu_table_controls_top20_melt[!grepl("NEG", otu_table_controls_top20_melt$SampleID),]
...
pdf("Outputs/Taxa_Stacked_Bar_Plots_for_Controls_neg.pdf", height = 9, width = 15)
